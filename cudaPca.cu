...
vector<double> calculateEigenvaluesCuda(const vector<vector<double>>& covMatrix){
    vector<double> flattenedMatrix = flatten(covMatrix);

    // Dimension of the covariance matrix
    int N = covMatrix.size(); // Assuming covMatrix is square

    // CUDA variables
    cusolverDnHandle_t handle;
    cusolverDnCreate(&handle);

    // Workspace and devInfo variables
    int *devInfo;
    ...
    cudaMemcpy(d_A, flattenedMatrix.data(), N * N * sizeof(double), cudaMemcpyHostToDevice);
    ...
    int work_size;
    cusolverDnDsyevd_bufferSize(handle, CUSOLVER_EIG_MODE_VECTOR, CUBLAS_FILL_MODE_UPPER, N, d_A, N, d_W, &work_size);

    // Allocate workspace
    double *work;
    cudaMalloc(&work, work_size);

    // Perform eigenvalue decomposition
    cusolverDnDsyevd(handle, CUSOLVER_EIG_MODE_VECTOR, CUBLAS_FILL_MODE_UPPER, N, d_A, N, d_W, work, work_size, devInfo);

    // Copy eigenvalues back to host
    vector<double> eigenvalues(N);
    cudaMemcpy(eigenvalues.data(), d_W, N * sizeof(double), cudaMemcpyDeviceToHost);
...
vector<vector<double>> pca(vector<vector<double>>& data) {
    ...
    vector<double> eigenvalues = calculateEigenvaluesCuda(covMatrix);
